...
{
    [CustomEditor(typeof(BFDOTweenAnimation))]
    public class BFDOTweenAnimationInspector : ABSAnimationInspector
    {
        enum FadeTargetType
        {
            CanvasGroup,
        ...
        static string[] _bfAnimationTypes;
        ...
        void OnEnable()
        {
            _src = target as BFDOTweenAnimation;
          ...
          //  _bfAnimationTypes = new string[Enum.GetNames(BFViewAnimType).Length]{Enum.GetNames(BFViewAnimType)};
...
//            _src.isValid = Validate(); // Moved down
            ...
            GUILayout.Space(2);
            GUILayout.BeginHorizontal();
            SerializedProperty _BFViewAnimType = serializedObject.FindProperty ("_BFViewAnimType");
            EditorGUILayout.PropertyField(_BFViewAnimType);

           
          //  _BFViewAnimType.
            //_BFViewAnimType.en
           // _src._BFViewAnimType = EditorGUILayout.Popup(Array.IndexOf(_bfAnimationTypes, _src._BFViewAnimType.ToString() ), _bfAnimationTypes); 
           // _src.forcedTargetType = (TargetType)Enum.Parse(typeof(TargetType), EditorGUILayout.EnumPopup(_src.animationType + " Target", _bfAnimationTypes).ToString());
        ...
        bool Validate()
        {
            ...
            if (_AnimationTypeToComponent.ContainsKey(_src.animationType)) {
                foreach (Type t in _AnimationTypeToComponent[_src.animationType]) {
                    ...
                    if (srcTarget != null) {
                        _src.target = srcTarget;
                        _src.targetType = BFDOTweenAnimation.TypeToDOTargetType(t);
                        return true;
                    }
                }
            }
